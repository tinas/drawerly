...
CSS selector for the element the drawer container is teleported to. The target must exist in the DOM before the container mounts.
...
```vue
<DrawerlyContainer :lock-scroll="false" />
```
...
Fallback content for drawers opened without a `component`. The scoped slot receives the drawer instance (with all its options and custom fields) and a function that closes that drawer:
...
```ts
// No component: renders through the default slot
drawerly.open({
  drawerKey: 'my-drawer',
  title: 'Hello',
})
...
When a `component` is provided, it is rendered instead of the slot.

## Events

### `drawer-opened`

Emitted when a drawer enters the stack. Payload: `{ key: DrawerKey }`.

### `drawer-closed`

Emitted when a drawer finishes closing, after its exit animation completes. Payload: `{ key: DrawerKey }`.

### `all-closed`

Emitted when the last drawer has finished closing and the stack is empty. No payload.

```vue
<DrawerlyContainer
  @drawer-opened="({ key }) => console.log('opened', key)"
  @drawer-closed="({ key }) => console.log('closed', key)"
  @all-closed="() => console.log('all closed')"
/>
```
...
The container is a Vue `<TransitionGroup>`: it applies `.drawerly-enter-active` while a drawer enters and `.drawerly-leave-active` while it leaves, and lets Vue's own transition timing decide when the element actually leaves the DOM. This means every path that removes a drawer, a single close, `closeAll()`, or a direct call on the instance, plays the exit animation, and body scroll stays locked for as long as a closing drawer is still visible. When `prefers-reduced-motion` disables animations, Vue detects the zero duration and finishes the transition immediately, so nothing gets stuck. See [Custom Animations](../unstyled-mode.md#custom-animations) for the CSS contract.
...
<div data-drawerly-root>
  <div data-drawerly-overlay
    ...
    <div data-drawerly-panel
         ...
         aria-modal="true"
         aria-label="...">
      <!-- Your component or slot content renders here -->
    </div>
  </div>
</div>
```
...
Teleport cannot render during SSR, so the stack is mounted client-side only.
